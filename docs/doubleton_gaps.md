...
|---|---|---|---|---|---|
| T | **3-2-4-5-6-1** in 22 of 24 L1·blog·modal cells *(the count was recorded as 23; the JSON gives 22/24, and 29/32 when burst is included — IT2a n5 L1/burst/modal are 3-2-4-5-1-6; corrected 2026-09-03 in the standalone report)*; words 2-3-4-5-6-1 / 3-2-4-5-6-1 | 5–27 / 720 | 0.6–1.7 | 0.8–1.4 ± 0.5–0.7 | 0.20–0.80 |
...
## 19. Alternative nesting patterns: does any other way of gathering the sheets do as well as stacking? (2026-09-03)

**Question.** §13–16 compared the S! fully-stacked sheet orders with one nested order, the binding. That leaves open whether some *other* nested arrangement — the same sheets nested in a different order, or a quire made of two or three smaller nested gatherings laid one after another — would describe the rare-material locality as well as the stacked optimum. Here every such pattern is a candidate: an order of the S sheets cut into consecutive sub-gatherings, each sub-gathering nested (its sheets' a-leaves outer→inner, then their b-leaves inner→outer). A sub-gathering of one sheet is that sheet read stacked, so the fully stacked orders (S blocks) and the fully nested ones (one block, the binding among them) are the two extremes of one space of S!·2^(S−1) page sequences: 192 for a four-sheet quire, 1 920 for M, 23 040 for T (quire B has 144 distinct sequences because its fourth "sheet" is the single leaf f13, f12 being lost, so a block that starts with it reads the same as stacking it before the block). Labels write blocks between bars, sheets outer→inner inside a block: `1|4|3|2` is stacked, `1432` nested, `14|32` two nested pairs.

**Metrics and a bias to declare.** `scripts/quire_order_nesting.py` (`quire_order_nesting_{A,B,C,M,T}.{json,log}`, summariser `quire_order_nesting_summary.py`, table `quire_order_nesting_table.py`). Because a nested block separates a sheet's two leaves, the within-sheet distances are no longer constant across candidates, so the pair sets change: L1 is the mean distance over *all* cross-page pairs of rare types (the §13 "stacked at all vs as bound" setting extended to the whole space); burst/blog/modal keep the §14 outlier→nearest-home term and add a home-cluster term (every home occurrence → its nearest other home occurrence), which is constant across the stacked orders (so their ranking there is §15's) and charges any pattern that splits a home sheet. That second family turned out to be **structurally biased toward stacked patterns**: on shuffled contents its winner is fully stacked in 76–100 % of draws on the n-grams (the home sheet is by definition the sheet holding most of a type's occurrences, and keeping that sheet's four pages contiguous is what minimises the home term whatever the content), and nested-written prose is never ranked first by burst/blog/modal in 27 controls. L1 has no such bias — its shuffled-content winners fall on the block counts in proportion to how many patterns each class has (fully nested 6–7 % of null winners in M against 120/1 920 = 6.3 % of the space; 12–17 % in the four-sheet quires against 24/192 = 12.5 %) and it does recover nested writing (below) — so **L1 is the metric for the class question** and the burst family is reported only as a check of the within-class ranking. Null: page contents permuted among the quire's slots (200 draws; 100 on T), best-of-all-patterns recomputed, and every statistic below compared with its own value on the shuffled winner.

**Power (prose on the quire's slots, three texts × three windows, words, L1).** Written as bound the binding ranks 1 of 192 in 2/9 (C), 1/9 (A), 3/9 (B), 0/9 of 1 920 (M) and 1/9 of 23 040 (T); written nested in a random sheet order, that order ranks 1 in 4/9, 0/9, 2/9, 0/9, 1/9; a random two-block pattern 1/9, 0/9, 1/9, 1/9, 1/9; a random stacked order 1/9, 1/9, 0/9, 1/9, 1/9. Median ranks 9–27 in every mode (T's stacked-random 134 of 23 040) *(T's control block in the JSON tallies the one word stream under all five unit labels — a pre-fix artefact; the summariser divides by five)*. So at these quire sizes the metric identifies the writing *pattern* about as well as it identified the writing *order* in §13 (a third of the time to the top ten), with no preference among the four kinds of pattern: when prose is written nested, a nested pattern wins; when it is written stacked, a stacked one wins.

**Manuscript.** Ranges over the five units (words, n 5–8); the fully nested class has S! members, the binding is one of them.

| quire (sheets, patterns) | transcr. | blocks in winner, words/n5/n6/n7/n8 | gain of full space over best stacked, sd (p) | rank of best stacked | rank of best fully nested | rank of binding | best nested − best stacked, sd (P chance) |
|---|---|---|---|---|---|---|---|
| A (4 sheets, 192) | IT2a | 3/4/3/3/3 | 0.00–0.13 (0.80–1.00) | 1–3 | 14–41 | 31–177 | +0.8–+1.2 (0.12–0.36) |
| A (4 sheets, 192) | RF1b | 4/3/4/3/3 | 0.00–0.08 (0.90–1.00) | 1–2 | 25–50 | 36–183 | +1.2–+1.4 (0.09–0.15) |
| B (4 sheets, 144) | IT2a | 2/4/4/4/4 | 0.00–0.79 (0.33–1.00) | 1–8 | 3–90 | 3–125 | -0.3–+3.0 (0.00–0.74) |
| B (4 sheets, 144) | RF1b | 2/4/4/4/4 | 0.00–0.69 (0.40–1.00) | 1–12 | 5–88 | 5–124 | -0.3–+2.9 (0.00–0.74) |
| C (4 sheets, 192) | IT2a | 3/3/4/4/4 | 0.00–0.32 (0.71–1.00) | 1–3 | 17–100 | 17–123 | +1.0–+1.9 (0.05–0.16) |
| C (4 sheets, 192) | RF1b | 3/3/4/4/4 | 0.00–0.02 (0.92–1.00) | 1–2 | 19–106 | 19–145 | +1.0–+2.2 (0.03–0.23) |
| M (5 sheets, 1920) | IT2a | 4/4/4/4/5 | 0.00–0.38 (0.71–1.00) | 1–13 | 650–818 | 1404–1862 | +2.3–+2.6 (0.00–0.07) |
| M (5 sheets, 1920) | RF1b | 5/4/3/4/5 | 0.00–0.41 (0.72–1.00) | 1–24 | 177–881 | 1569–1887 | +1.5–+2.7 (0.03–0.10) |
| T (6 sheets, 23040) | IT2a | 4/6/6/6/6 | 0.00–0.18 (0.90–1.00) | 1–5 | 1750–10535 | 13942–22405 | +1.9–+4.7 (0.01–0.05) |
| T (6 sheets, 23040) | RF1b | 5/6/6/6/6 | 0.00–0.16 (0.95–1.00) | 1–2 | 3588–8434 | 16868–21492 | +3.0–+4.6 (0.01–0.02) |

*Reading of the columns.* "Gain of full space over best stacked" is how far below the best stacked order the overall winner lies, in candidate sd, with the fraction of shuffled contents whose winner gains at least as much: a gain at p ≥ 0.7 is the ordinary selection effect of searching 8–32 times more candidates. "Best nested − best stacked" is the margin by which the best of the fully nested orders trails the best stacked one, with the probability that shuffled contents produce a margin at least that large.

**Result.** **Quire T (23 040 patterns) is the decisive case:** the fully stacked `3|2|4|5|6|1` — the §16 un-folded optimum — wins the whole space outright on every n-gram unit and both transcriptions (p 0.01 against the shuffled winner at 100 draws), the best fully nested order ranks 5 007–10 535 (IT2a) and 6 146–8 434 (RF1b), the binding 19 180–22 405 / 17 902–21 492, and the nested class trails the stacked optimum by 3.4–4.7 sd where shuffled contents give +0.7–0.9 (P(chance) 0.01 on all eight n-gram cells, 0.02–0.05 on words). Nothing in the full space improves on the best stacked order: the shuffled winner typically beats its own best stacked order by 0.9–1.2 sd through the wider search, the manuscript's by 0.00 (n-grams) and 0.16–0.18 (words, p 0.90–0.95). In every quire and both transcriptions the overall winner is fully stacked or a pattern with one nested pair among otherwise stacked sheets (A's `1|42|3`, M's `14|5|2|3`, C's `2|13|4` on words), and where it is not fully stacked it beats the best stacked order by 0.02–0.4 sd — exactly what the enlarged search gains on shuffled contents (p 0.7–1.0 in all n-gram cells; the only sub-0.5 values are quire B's words, where nothing is significant at all, p_best 0.4–0.7). No fully nested order comes close: the best of them ranks 14–106 of 192 in the herbal quires (except B's words, again a null cell), 177–881 of 1 920 in M, 5 007–10 535 of 23 040 in T, and the binding itself ranks lower still (31–183, 1 404–1 887, 17 902–22 405). The fully nested class trails the stacked optimum by 1–3 sd, which shuffled contents reproduce in only 3–15 % of draws on most n-gram cells (M IT2a 0.005–0.07, C 0.03–0.23, A 0.09–0.36; B's n5 0.005). Two-block patterns — the natural "two smaller gatherings" reading — never win on an n-gram unit anywhere.

**Reading.** The §13–15 comparison was not a straw man: the sheets-together property that the stacked orders share is what the rare material rewards, and no other way of gathering the same sheets — reordered nesting, two or three nested sub-gatherings, or a mixed pattern — recovers it. Within the stacked class the T chain of §15 stands as recorded (adjacency only, §16). Caveat: the burst-family metrics cannot be used for this question at all (bias above), and a nested pattern whose leaves happen to coincide with the stacked adjacency (a sheet nested inside a one-sheet block is just that sheet stacked) is already in the space; what is *not* in the space is inverted folding (§13's extension) combined with nesting, which nobody has proposed.
