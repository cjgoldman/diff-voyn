...
> **Record status (banner added 2026-09-01):** Phases 0–6, narrative frozen at the Phase-6 result of 2026-08-24 (the manuscript abstains, 0/87 cells). Everything in §1–§8 is the record as of that date and is left intact; §8/§9's next steps and the "selection bias" account of the Borg polish are annotated in place below. Work after 2026-08-24 is **not** narrated here: a fifth cipher head (word-homophonic, `heads/wordhom.py`, `docs/wordhom_study.md`); the alternating n-gram ↔ diffusion loop (`docs/alt_loop_plan.md` §7, `docs/altloop_vms_plan.md`); the hapax-wildcard → anneal solver of record with a findability wall of ≈ 4 tokens per type (`docs/alt_loop_plan.md` §8.4–8.6, `docs/project_status.md` §3); the manuscript re-confirmed NOISE on 2026-08-26 (72/72), 2026-08-29 (24/24) and 2026-08-31 (8/8); manuscript-shaped control batteries including the strict-negative voynichesque twins (`docs/alt_loop_plan.md` §10, `docs/voynichesque_nocontent_restart.md`); the judge studies (`docs/judge_alternatives.md`, nothing adopted); and the Polygraphia quarantine (`docs/polygraphia_status.md` §5). **Current project position: `docs/project_status.md`.**
...
*[Re-classification 2026-08-31 (`docs/voynichesque_nocontent_restart.md`, `analysis/phase6/controls_nocontent/report.json`; `docs/project_status.md` §1 "Phase-6 acceptance", §5 item 6): the nine gibberish instances above are generated from real held-out text. Their strict-negative twins — the same generator, seeds and alphabets on a letter-shuffled source — run through this identical pipeline abstain 9/9 (0/66 cells language-like); the homophonic twin scores 0.27 bits/char lower than its real-text partner on all 27 pairs (real 0.85–1.51 → twin 0.55–1.24), and the failing instance (italian t1, 1.43/1.51/1.50) deflates to 1.15/1.24/1.14. The maximum twin margin is 1.40. So the failing bar was tested on a non-negative: real-text voynichesque is a wrong-hypothesis control, not a strict negative. The recorded FAIL is left as the frozen-rule outcome — this is a re-classification of the control, not a pass.]*
...
**A negative finding about the Phase-5 outer tier.** The discrete "ELBO polish" that improved keys on the Phase-5 synthetics was applied to Borg, accepted on all six cells, and made the decipherment *worse* every time (median page error 11% → 22%, 25 of 55 symbols reassigned). The mechanism is selection bias: each sweep picks the best of ~1,400 cheaply estimated moves, so the winner is systematically over-rated, and the one careful confirmation at the end compares only the final key against the start and cannot undo a chain of individually biased steps. This is a scale effect the small Phase-5 ciphers could not reveal. It did *not* produce the manuscript abstention — re-scoring the best VMS cells with their pre-polish keys gives margins of 0.83–0.84 against 1.05–1.11 after, both far short of 1.5 — but it must be fixed (per-move confirmation at full budget, or a bias-corrected selection rule) before the polish is reused on anything larger than the synthetics. *[Superseded 2026-08-25: the dominant cause was the homophonic choice-bits term in the polish objective, not selection bias — the judge dislikes `e` → `z` (+0.087 bits/char) while the choice term rewards it (−0.243); polishes now run on the ELBO alone by default (code raises otherwise), and a pure-ELBO polish holds Borg (0.1195 → 0.1194). A noise-robust racing selection rule (`ladder.race_polish`) exists as well; the full race study was not run. See `docs/race_polish_plan.md` §7, `docs/phase6_status.md` §6.6 correction paragraph, `docs/project_status.md` §5 item 4.]*

**Acceptance roll-up.** `scripts/phase6_check.py`: 6.1, 6.2, 6.4, 6.5 and the freeze discipline all PASS; 6.3 gibberish abstention **FAIL** (0.89); 6.3 positives WARN (one false abstention); 6.6 Borg WARN (SER target missed, language recovered); 6.6 BnF WARN (unavailable). **Overall: FAIL on one P0 sub-criterion, reported as such.** *[See the 2026-08-31 re-classification note under the control battery above: the failing instance is a content-inflated non-negative; the verdict stands as recorded.]*
...
| A9 | MIT release | **planned** | Code is in a single repository with pinned external dependencies and fingerprinted model files; not yet released. *[Still open as of 2026-09-01.]* |
...
- **The abstention threshold sits in an overlap band.** The 1.5-bit structure-margin cut is bracketed by a true decipherment at 1.49 and a gibberish instance at 1.51 in a battery of 39. The manuscript verdict is insensitive to it (any cut in 1.26–1.48 gives the same answer), but the rule's operating point is known only to the precision a 39-instance battery allows. *[Updated 2026-08-31: the 1.51 instance is content-inflated (strict-negative twin 1.24); strict gibberish peaks at 1.40, so the corridor safe against a strict negative is ≈ 1.41–1.48. The battery was enlarged 2026-08-29/31 (`docs/alt_loop_plan.md` §10; strict twins) — `docs/project_status.md` §4.]*
- **The outer-tier polish has a demonstrated failure mode at scale** (selection bias on Borg). It did not produce the manuscript result, but it is not fit for reuse until fixed. *[Superseded 2026-08-25: the failure was mostly the choice-bits term in the polish objective; ELBO-only polishing is now the code default and holds Borg — `docs/race_polish_plan.md` §7; `docs/project_status.md` §5 item 4.]*
...
*[Status of this list as of 2026-09-01 is appended to each item in brackets; the list itself is the 2026-08-24 text. The canonical open-items register is `docs/project_status.md` §6.]*

1. **Revise the abstract** to match what was found: A7 becomes an abstention result (no language-like decipherment; the Naibbe word-grammar finding on Currier A as the one structural positive), A3 (hybrid, not end-to-end), A2a (curriculum, not architecture), A2b (description length, not raw ELBO), A10 (three languages). *[Status 2026-09-01: **open** — `reference_docs/` is intentionally left byte-identical as the pre-registration record; the correction is carried by §6–7 of this document and `docs/project_status.md` §5 item 15.]*
2. **Fix the outer tier** before any reuse: per-move full-budget confirmation or a bias-corrected selection rule for the ELBO polish (the Borg finding). *[Status 2026-09-01: **done 2026-08-25**, in a different form than anticipated — the cause was the choice-bits term in the polish objective, not selection bias; polishes now run on the ELBO alone by default (enforced in code) and hold Borg (0.1195 → 0.1194). The racing selection rule (`ladder.race_polish`) exists; the full race study of `docs/race_polish_plan.md` §3 was **not run**. See `docs/race_polish_plan.md` §7.]*
3. **Sensitivity studies the abstention deserves**: a sweep over EVA tokenizations (glyph units, benched gallows as single symbols); a larger control battery — or a length-matched margin normalisation — around the 1.26–1.51 overlap band so the threshold's operating point is better known. *[Status 2026-09-01: split. (a) EVA tokenization sweep — **open, never started**. (b) Larger control battery — **partly done**: a manuscript-shaped battery for the word-homophonic solver of record (12 negatives + 6 cross-language, all NOISE ≤ 0.48; `docs/alt_loop_plan.md` §10, 2026-08-29/30) and the strict-negative voynichesque twins through the Phase-6 controls pipeline (ceiling 1.40, 9/9 abstain, safe corridor ≈ 1.41–1.48; `docs/voynichesque_nocontent_restart.md`, 2026-08-31). Length-matched margin normalisation — **open, not started**.]*
4. **Anchors**: obtain the DECODE-aligned Borg transcription and the BnF fr2988 transcription for a like-for-like symbol-error comparison. *[Status 2026-09-01: **open**. A Polygraphia (Trithemius) digitization was scoped as a replacement verbose-tier anchor and is now quarantined from every control workflow after producing the pipeline's first false calls (`docs/polygraphia_status.md` §5).]*
5. Optional strengthening: resume the seed replication (4.7) so ranking stability across independently trained models can be stated. *[Status 2026-09-01: **still paused** since 2026-08-22 at resumable checkpoints (`docs/phase4_status.md` §4.7); every number in the project remains one 85M evaluator.]*
6. **Two control experiments the current argument owes a reviewer.** *[Status 2026-09-01: **launched 2026-08-24, paused by the user the same day at 18:05**. 6b rung 1 is complete — language recovery A 0.994 vs pooled-search B 0.978 / C 0.972 at ≥ 200 chars, every change Latin → German, and a pooled Latin solve rate of 0.933, i.e. the asymmetry is the Latin text itself, not the per-hypothesis search (`analysis/control6/control6b_rung1_report.md`); 6b rung 2 is solved but unscored; 6a (no-wrong-key curriculum ablation) stopped at 25M step 3600/6000 with the 85M run killed. The planned write-up `docs/control6_experiments.md` was never written. See `docs/project_status.md` §6.]* Both address the two-tier search of §5 (Phase 5, finding 1) — a cheap classical search proposes keys under each language hypothesis, the diffusion model only judges — whose justification is so far mechanistic rather than experimental. Both run on the existing synthetic suites with known answers, not on the manuscript, and neither touches the frozen evaluator used for every Phase-6 number.
