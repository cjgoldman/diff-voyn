...
## 3. Search: the objective is right, the key is not findable at the manuscript's type/token ratio
...
varied. The search is essentially perfect down to ~8 tokens per type and
fails outright at 3.5:
...
The break is sharp: essentially perfect recovery at ≥ 8 tokens per type,
a third to a half of the map wrong at 6.7–6.8, more than half wrong at
≤ 5.7 — and a true cipher with Currier A's or B's own type/token shape
(A: 10.8k tokens / 3.6k types = 3.0; B: 23k / 5.0k = 4.6) comes back with
letter SER 0.57–0.76, hundreds of rule violations and a decode at 3.2–3.7
bits/char — the same numbers the manuscript produces (§5). The search
...
More text per type is the only lever, and the manuscript's is fixed.
