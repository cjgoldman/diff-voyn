...
class CharARLM(nn.Module):
    """``forward(ids [B, L]) -> next-char logits [B, L, VOCAB_SIZE]`` where
    ``logits[:, i]`` predicts ``ids[:, i]`` from BOS + ``ids[:, :i]``."""
    ...
    def nll_nats(self, ids: torch.Tensor) -> torch.Tensor:
    ...
    @torch.no_grad()
    def nll_bits_per_char(self, ids: torch.Tensor) -> torch.Tensor:
        """Per-window bits/char, [B] — the §5b.3 ``NLL_AR`` term."""
        return self.nll_nats(ids) / (ids.shape[1] * math.log(2.0))

    def n_params(self) -> int:
        return sum(p.numel() for p in self.parameters())


def ar_loss(model: CharARLM, ids: torch.Tensor) -> torch.Tensor:
    """Mean next-char cross-entropy in nats/char (training objective)."""
    return model.nll_nats(ids).sum() / ids.numel()
